 ...
 */
import { PersonaData, rareCombos, personaMap, specialCombos, arcana2Combos } from '../data';
import { customPersonaList, special2Combos, getResultArcana } from './DataUtil';

export default class FusionCalculator {
  ...
  public getAllResultingRecipesFrom(persona: PersonaData): Recipe[] {
    ...
    for (let i = 0; i < customPersonaList.length; i++) {
      const result = this.fuse(persona, customPersonaList[i]);
      if (result !== null) {
        const recipe = {
  ...
  private getArcanaRecipes(arcana: string): Recipe[] {
    const recipes: Recipe[] = [];
    const arcanaCombos = arcana2Combos.filter(x => x.result === arcana);
    ...
    for (let i = 0; i < rarePersonae.length; i++) {
      const rarePersona = personaMap[rarePersonae[i]];
      const personae = this.personaeByArcana[arcana];
